...
},
{
  type:'input',
  name:'target',
  message:'[>] Insert Link Target (https://wwwinstagram.com/p/1234abcd) :',
  validate: function(value){
    if(!value) return 'Can\'t Empty';
    return true;
  }
},
{
	type:'input',
	name:'customCaption',
	message:'[>] Pake Custom Caption Gk? (Y/N)',
	validate: function(value){
...
const Excute = async function(User, target, customCaption){
  try {
  	const customCaptionText = fs.readFileSync("cc.txt","utf-8");
...
console.log(chalk`
  ...
  ——————————————————     [FAQ]     ————————————————————
  [Q] Cara Ganti Custom Caption?
  [A] Edit file cc.txt

  [Q] Klo gk mau pake Custom Caption?
  [A] Pas pilihan custom caption pilih {bold.yellow N} saja iyh
  ——————————————————  [THANKS TO]  ————————————————————
  [✓] RMT BY MAS OKKY (@masokky_)
  [✓] CODE BY CYBER SCREAMER CCOCOT (ccocot@bc0de.net)
  [✓] FIXING and TESTING BY SYNTAX (@officialputu_id) & IsaNoorendra @isanoorendra_
